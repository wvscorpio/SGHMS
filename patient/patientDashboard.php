...
$patient_id = "PAT003"; // Fallback

if ($current_username) {
    $stmt_pat = $conn->prepare("SELECT patientID FROM patient WHERE name = ?");
    if ($stmt_pat) {
        $stmt_pat->bind_param("s", $current_username);
        $stmt_pat->execute();
        $res_pat = $stmt_pat->get_result();
        if ($row_pat = $res_pat->fetch_assoc()) {
            $patient_id = $row_pat['patientID']; 
        }
        $stmt_pat->close();
    }
}
...
// --- 2. FETCH SCHEDULES ---
$all_schedules = [];
$sched_query = "SELECT doctorID, dayOfWeek, startTime FROM doctor_schedule";
...
if (isset($_POST['save_appointment'])) {
    ...
    if (empty($app_id)) {
        // INSERT
        $new_id = generateAppointmentID($conn);
        $stmt = $conn->prepare("INSERT INTO appointment (appointmentID, appointmentDate, appointmentTime, reason, status, patientID, doctorID) VALUES (?, ?, ?, ?, 'Pending', ?, ?)");
        $stmt->bind_param("ssssss", $new_id, $app_date, $app_time, $reason, $patient_id, $doctor_id);
    } else {
        // UPDATE
        ...
        $stmt->bind_param("ssssss", $doctor_id, $app_date, $app_time, $reason, $app_id, $patient_id);
    }
...
if (isset($_POST['delete_id'])) {
    ...
    $stmt->bind_param("ss", $delete_id, $patient_id);
    $stmt->execute();
    $stmt->close();
}
...
<head>
    ...
    <style>
        .appointment-container { display: flex; flex-direction: column; gap: 20px; padding: 20px 40px; }
        .app-card { background: white; border-radius: 12px; padding: 25px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); border: 1px solid #eee; width: 100%; box-sizing: border-box; position: relative; }
        ...
        .status-badge { background: #4e73df; color: white; padding: 5px 15px; border-radius: 20px; font-size: 0.85em; font-weight: 600; }
        .card-details-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 20px; }
        @media (max-width: 768px) { .card-details-grid { grid-template-columns: 1fr 1fr; } }
        ...
        .reason-section { border-top: 1px solid #f0f0f0; padding-top: 15px; margin-bottom: 15px; }
...
<body>
    ...
    <div class="header" style="display: flex; justify-content: space-between; align-items: center; padding: 20px; background-color: white; border-bottom: 1px solid #ddd;">
        <div>
    ...
    <div class="button_add_apt" style="justify-content: space-between; align-items: center; margin-top: 20px;">
    ...
    <div class="appointment-container">
        <?php if ($myAppointments && $myAppointments->num_rows > 0): ?>
            <?php while($row = $myAppointments->fetch_assoc()): ?>
                <div class="app-card">
                    <div class="card-header-row">
                        ...
                        <span class="status-badge" style="background-color: <?= ($row['status'] == 'Pending') ? '#f6c23e' : '#4e73df'; ?>">
                    ...
                    <div class="card-details-grid">
                        <div class="detail-group">
                            ...
                            <span class="detail-value"><?= htmlspecialchars($row['appointmentID']); ?></span>
                        </div>
                        <div class="detail-group">
                        ...
                        <div class="detail-group">
                            ...
                            <span class="detail-value"><?= date('h:i A', strtotime($row['appointmentTime'])); ?></span>
                        </div>
                    </div>
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <p style="padding: 20px; text-align: center;">No appointments found.</p>
        <?php endif; ?>
    </div>
            ...
            <form method="post">
                ...
                <div class="form-group">
                    ...
                    <select name="time" id="time" required>
                        <option value="" disabled selected>Please select Doctor & Date first</option>
                    </select>
                </div>
    ...
    <script>
        ...
        function updateTimeSlots() {
            ...
            const availableTimes = allSchedules.filter(s => s.docID == doctorID && s.day == selectedDay);
            ...
            if (availableTimes.length > 0) {
                ...
                let option = document.createElement("option");
                option.text = "No slots available on " + selectedDay;
                timeSelect.add(option);
            }
        }
        ...
        function openModal() {
            ...
            document.getElementById("time").innerHTML = '<option value="" disabled selected>Please select Doctor & Date first</option>';
        ...
        function editAppointment(id, docID, date, time, reason) {
            ...
            updateTimeSlots();
            setTimeout(() => { document.getElementById("time").value = time; }, 100);
